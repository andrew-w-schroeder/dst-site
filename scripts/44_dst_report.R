...
make_proj_tbl <- function(pred) {       # same columns as 40_dst_model.R section 11, from (possibly refreshed) pred
  ...
  if ("proj_base" %in% names(pred)) t <- t %>% mutate(`Δ Proj` = signed(pred$proj - pred$proj_base, 2), .after = Proj) %>%
...
compare_gloss <- tribble(~term, ~definition,
  ...
  "Δ Opp implied", "change in the opponent's Vegas implied points since the weekly model run (negative = good for this D/ST)",
  "Δ Proj", "change in the projection since the weekly model run, driven only by Vegas line moves",
...
describe <- function(x) {
  x2 <- sub("^(ESPN|Yahoo|FFPC) (proj|rank|P\\(top 8\\))$", "<System> \\2", x)
...
html_table <- function(df, id = NULL, sortable = FALSE, left = 1, rank_cols = character(), tips = TRUE, raw_cols = character()) {
  ...
  rows <- map_chr(seq_len(nrow(df)), function(i) paste0("<tr>", paste0(map_chr(seq_along(df), function(k) {
...
sys_tab <- function(p) {
  pt <- make_proj_tbl(p$pred); ac <- if ("Δ Proj" %in% names(pt)) "Δ Proj" else "Proj"
  ...
  paste0(sprintf("<p class='rules'><b>%s scoring:</b> %s</p>", esc(p$SC$label), esc(p$SC$rules)),
         ...
         html_table(pt, id = paste0("t_", p$system), sortable = TRUE, left = if (refreshed) 5 else 4, rank_cols = "Rank", raw_cols = "Range"),
...
tabs <- c(list(Compare = paste0(
  ...
  "Click a column header to sort. Green = top 8 in that system, red = bottom 8. P(top 8) = chance of actually finishing top 8 this week; see each format's tab for score ranges.</p>",
  html_table(cmp, id = "t_compare", sortable = TRUE, left = if (refreshed) 4 else 3, rank_cols = rank_cols),
...
table.l1 td:nth-child(-n+1),table.l3 td:nth-child(-n+3),table.l4 td:nth-child(-n+4),table.l5 td:nth-child(-n+5){text-align:left}
